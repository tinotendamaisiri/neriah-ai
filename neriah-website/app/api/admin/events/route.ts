...
// ── Cookie verifier (mirrors curriculum proxy) ────────────────────────────────
...
// ── GET /api/admin/events  → upstream /api/admin/events/list ──────────────────
export async function GET(req: NextRequest) {
  if (!await verifyAdminCookie(req)) return unauthorized();

  const apiKey = process.env.ADMIN_API_KEY ?? '';
  const { search } = new URL(req.url);
  const upstream = `${CLOUD_FN_BASE}/admin/events/list${search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Upstream events service unreachable', detail: String(err) },
      { status: 502 },
    );
  }
}
