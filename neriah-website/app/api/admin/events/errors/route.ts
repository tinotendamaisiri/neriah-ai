...
// ── GET /api/admin/events/errors  → upstream /api/admin/events/errors ─────────
export async function GET(req: NextRequest) {
  if (!await verifyAdminCookie(req)) return unauthorized();

  const apiKey = process.env.ADMIN_API_KEY ?? '';
  const { search } = new URL(req.url);
  const upstream = `${CLOUD_FN_BASE}/admin/events/errors${search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Upstream errors service unreachable', detail: String(err) },
      { status: 502 },
    );
  }
}
