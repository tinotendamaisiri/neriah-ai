...
// ── GET /api/admin/events/funnel  → upstream /api/admin/events/funnel ─────────
// Forwards `id` and `days` query params to the upstream.
export async function GET(req: NextRequest) {
  if (!await verifyAdminCookie(req)) return unauthorized();

  const apiKey = process.env.ADMIN_API_KEY ?? '';
  const { search } = new URL(req.url);
  const upstream = `${CLOUD_FN_BASE}/admin/events/funnel${search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Upstream funnel service unreachable', detail: String(err) },
      { status: 502 },
    );
  }
}
