...
// ── GET /api/admin/events/ai_usage  → upstream /api/admin/events/ai_usage ─────
...
export async function GET(req: NextRequest) {
  ...
  const upstream = `${CLOUD_FN_BASE}/admin/events/ai_usage${search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Upstream AI usage service unreachable', detail: String(err) },
      { status: 502 },
    );
  }
}
