...
// ── GET /api/admin/events/trace  → upstream /api/admin/events/trace ───────────
// Forwards `phone`, `user_id`, or `trace_id` query params to the upstream.
export async function GET(req: NextRequest) {
  if (!await verifyAdminCookie(req)) return unauthorized();

  const apiKey = process.env.ADMIN_API_KEY ?? '';
  const { search } = new URL(req.url);
  const upstream = `${CLOUD_FN_BASE}/admin/events/trace${search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Upstream trace service unreachable', detail: String(err) },
      { status: 502 },
    );
  }
}
