...
export default function MarkingScreen() {
  ...
  const handleCapture = async (imageUri: string) => {
    ...
    try {
      ...
      // Network error (no HTTP status) — queue for later instead of losing the scan
      const isNetworkError = !err?.status && !err?.response?.status;
      if (isNetworkError) {
        try {
          await enqueue({
            image_uri: imageUri,
          ...
          Alert.alert(
            'Saved for later',
            'No connection — this scan has been queued and will upload automatically when you reconnect.',
          );
        } catch {
          showError(err);
        }
